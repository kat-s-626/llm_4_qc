...
def format_prompt(data_item: Dict[str, Any], eos_token: str = "") -> Dict[str, str]:
    if data_item.get(DATASET_MSB_MEASUREMENT_PROBABILITIES) is not None:
        """
        ...
        if data_item.get("problem") is not None and data_item.get("generated_solution") is not None:
