...
for filepath in grover_files:
    try:
        ...
        grover_overall_acc.append(data['overall_reasoning'])
...
for filepath in universal_files:
    try:
        data = parse_accuracy_file(filepath)
        universal_overall_acc.append(data['overall_reasoning'])
    except FileNotFoundError:
        print(f"Warning: {filepath} not found, using default value")
...
bars1 = ax.bar(x_pos - bar_width/2, grover_overall_acc, bar_width, 
               label='Grover Gate', color=ours_primary, 
...
plt.savefig('reasoning_accuracy_barchart.pdf', dpi=600, bbox_inches='tight')
plt.savefig('reasoning_accuracy_barchart.png', dpi=600, bbox_inches='tight')
print("Saved: reasoning_accuracy_barchart.pdf")
print("Saved: reasoning_accuracy_barchart.png")
plt.show()
