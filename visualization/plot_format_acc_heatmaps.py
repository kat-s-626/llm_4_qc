...
# Parse Grover Gate criteria data (7 criteria x 4 test sets)
grover_criteria_matrix = []
for filepath in grover_files:
    try:
        data = parse_accuracy_file(filepath)
        grover_criteria_matrix.append(data['format_criteria'])
    except FileNotFoundError:
        print(f"Warning: {filepath} not found, using default values")
        grover_criteria_matrix.append([0.0] * 7)

grover_criteria_matrix = np.array(grover_criteria_matrix).T  # Transpose to get (7 x 4)

# Parse Universal Gate criteria data (7 criteria x 4 test sets)
universal_criteria_matrix = []
for filepath in universal_files:
    try:
        data = parse_accuracy_file(filepath)
        universal_criteria_matrix.append(data['format_criteria'])
    except FileNotFoundError:
        print(f"Warning: {filepath} not found, using default values")
        universal_criteria_matrix.append([0.0] * 7)

universal_criteria_matrix = np.array(universal_criteria_matrix).T  # Transpose to get (7 x 4)
...
plt.savefig('format_accuracy_heatmaps.pdf', dpi=1000, bbox_inches='tight')
plt.savefig('format_accuracy_heatmaps.png', dpi=1000, bbox_inches='tight')
print("Saved: format_accuracy_heatmaps.pdf")
print("Saved: format_accuracy_heatmaps.png")
plt.show()
